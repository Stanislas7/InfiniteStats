...
export default function UsernameForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (trimmedUsername) {
      router.push(`/stats/${encodeURIComponent(trimmedUsername)}`);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-2xl mx-auto relative">
      <input
        ...
        className="w-full px-4 py-2 pr-12 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
        required
      />
      <button
        type="submit"
